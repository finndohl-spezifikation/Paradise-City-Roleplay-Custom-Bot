...
client.once('ready', async () => {
  ...
  const commands = [
    new SlashCommandBuilder()
      .setName('delete')
      .setDescription('Löscht Nachrichten in diesem Kanal (max. 200, egal wie alt)')
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (commandName === 'delete') {
    ...
    // Alle Nachrichten holen (bis zu anzahl)
    const fetched = await interaction.channel.messages.fetch({ limit: anzahl }).catch(() => null);
    if (!fetched || fetched.size === 0)
      return interaction.editReply({ content: '⚠️ Keine Nachrichten gefunden.' });

    const jung = fetched.filter(m => Date.now() - m.createdTimestamp < TWO_WEEKS);
    const alt  = fetched.filter(m => Date.now() - m.createdTimestamp >= TWO_WEEKS);

    let deleted = 0;

    // bulkDelete für Nachrichten < 14 Tage (schnell)
    if (jung.size >= 2) {
      const res = await interaction.channel.bulkDelete(jung, true).catch(() => null);
      deleted += res?.size || 0;
    } else if (jung.size === 1) {
    ...
    // Ältere parallel löschen (so schnell wie möglich)
    if (alt.size > 0) {
      const results = await Promise.allSettled(alt.map(m => m.delete().catch(() => {})));
...
client.on('interactionCreate', async (interaction) => {
  if (!interaction.isButton() || interaction.customId !== 'captcha_start') return;
  ...
  const CAPTCHA_FILE = path.join(DATA_DIR, 'captcha_tokens.json');
  let toks = {};
  try { toks = JSON.parse(fs.readFileSync(CAPTCHA_FILE, 'utf8')); } catch {}
  const now = Date.now();
  // Clean expired tokens
  Object.keys(toks).forEach(k => { if (toks[k].expiresAt < now) delete toks[k]; });
  // Check for existing active token
  const existingTok = Object.entries(toks).find(([,v]) => v.userId === interaction.user.id && v.expiresAt > now);
  let tok;
  if (existingTok) {
    tok = existingTok[0];
  } else {
    tok = require('crypto').randomBytes(16).toString('hex');
    toks[tok] = { userId: interaction.user.id, guildId: interaction.guild.id, expiresAt: now + 15 * 60 * 1000, attempts: 0 };
    try { fs.writeFileSync(CAPTCHA_FILE, JSON.stringify(toks, null, 2)); } catch (e) { console.error('Captcha token save error:', e.message); }
  }

  const domain = (process.env.RAILWAY_PUBLIC_DOMAIN || (process.env.REPLIT_DOMAINS || '').split(',')[0] || 'localhost:8080');
  const link = 'https://' + domain + '/verify/' + tok;
