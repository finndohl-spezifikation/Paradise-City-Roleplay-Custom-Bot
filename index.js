  ...
  client.once('ready', async () => {
    try {
      ...
      await ch.send({embeds:[new EmbedBuilder()
        ...
        .addFields(
          {name:'💰 Beute',value:'12.000 – 22.000 
  ...
  client.once('ready', async () => {
    try {
      ...
      await ch.send({ embeds: [new EmbedBuilder()
        ...
        .addFields(
          {name:'💰 Beute',value:'55.000 – 73.000 
    ...
    client.login(process.env.DISCORD_TOKEN)
      ...
      .catch(err => {
        ...
        setTimeout(() => loginWithRetry(attempt + 1), wait);
          ...
          {name:'🔧 Benötigte Items',value:'**Brechstange** *(10 Min.)* → 🔨〢𝘉𝘢𝘶𝘮𝘢𝘳𝘬𝘵\n**Automaten Sprengstoff** *(5 Min.)* → 👥〢𝘚𝘤𝘩𝘸𝘢𝘳𝘻𝘮𝘢𝘳𝘬𝘵',inline:false},
...
// ─── BAR-RAUB SYSTEM ──────────────────────────────────────────────────────────
{
  ...
  client.once('ready', async () => {
    try {
      const d = _loadBar(); if (d._infoEmbedSent) return;
      const ch = await client.channels.fetch(BAR_INFO_CH).catch(() => null); if (!ch) return;
      await ch.send({ embeds: [new EmbedBuilder().setColor(0xf59e0b)
        .setTitle('🍺 Bar-Raub — Informationen')
        .setDescription(
        ...
        .setFooter({ text: 'Paradise City Roleplay • Raubüberfälle' }).setTimestamp()] });
      d._infoEmbedSent = true; _saveBar(d);
    } catch(e) { console.error('[BAR-INFO]', e.message); }
  });
...
client.on('interactionCreate', async (interaction) => {
  ...
  const typ      = interaction.options.getString('typ');       // 'atm' | 'shop'
  const target   = interaction.options.getUser('spieler');
  const typLabel = typ==='bar'?'Bar-Raub':typ==='shop'?'Shop-Raub':'ATM-Raub';
  const nType    = typ==='bar'?'bar_raub':typ==='shop'?'shop_raub':'atm_raub';
  const rFile    = path.join(DATA_DIR, typ==='bar'?'bar_raub.json':typ==='shop'?'shop_raub.json':'atm_raub.json');
  function _lrf(){ try{return JSON.parse(fs.readFileSync(rFile,'utf8'));}catch{return{};} }
  function _srf(d){ fs.writeFileSync(rFile,JSON.stringify(d,null,2),'utf8'); }
