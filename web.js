...
module.exports = function startWebServer(client, DATA_DIR, lapdTokens = new Map(), shopCb = {}) {
  ...
  // ── GET /api/members — Servermitglieder für Picker ─────────────────────────
    ...
    // ── POST /api/darknet/discord/token ───────────────────────────────────────
    app.post('/api/darknet/discord/token', express.json(), async (req, res) => {
      const adminSecret = process.env.DARKNET_ADMIN_SECRET || 'darknet_admin_2025';
      const secret = req.headers['x-darknet-secret'] || req.body?.secret || req.query?.secret;
      if (secret !== adminSecret) return res.status(403).json({ error: 'Forbidden' });
...
.err-box{background:#1c0a0a;border:1px solid #f85149;color:#f85149;padding:10px 14px;border-radius:8px;margin-bottom:18px;font-size:.85em;text-align:center}
  ...
  // ── Inhaber Dashboard ────────────────────────────────────────────────────────
  try { require('./dashboard').init(app, express, DATA_DIR, client); } catch(e) { console.error('[DASHBOARD]', e.message); }
...
module.exports.darknetTokens = darknetTokens;
